 ...
 * and attributed to `kvlogin`.
 *
 * Event namespace: `kvlogin/<action>`
 * Every handler returns { ok: boolean, data?: unknown, error?: string, status?: number }
 *
 * API base URL: configurable via config.baseUrl, defaults to 'http://47.110.80.47:8988'
 */
...
type PluginFn = ((ctx: HostCtx, config?: { name?: string; baseUrl?: string }) => void) & {
  inject?: string[]
}
...
const DEFAULT_BASE_URL = 'http://47.110.80.47:8988'
const NS = 'kvlogin'

// ── Plugin ─────────────────────────────────────────────────────────

const plugin = function plugin(ctx: HostCtx, config?: { name?: string; baseUrl?: string }): void {
