 ...
 *   4. Watches the plugins directory for .ts/.tsx/.json changes
 *   5. On change: rebuilds the affected plugin + uploads the new zip
 *      (host's runId dedup hot-replaces the old run — no restart needed)
 */
...
async function rebuildAndUpload(pluginId) {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      execFile('node', [join(__root, 'scripts', 'build.mjs'), pluginId], { cwd: __root }, (err, stdout, stderr) => {
        if (err) {
    ...
    // Upload to running host (hot-replace via dedup)
