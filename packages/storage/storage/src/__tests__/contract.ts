 ...
 *
 * This file is copied from the dsh storage family contract suite
 * (`deepseek-harness/packages/storage/storage/tests/contract.ts`) and lives
 * in each backend package's `src/__tests__/` (the JSON and SQLite backend
 * packages each carry one copy, because cross-package relative imports are
 * not wired in this workspace).
 * @module
 */

import { describe, expect, it } from 'vitest'
import type { KvUnitDescriptor, StorageBackend } from '../backend.js'
