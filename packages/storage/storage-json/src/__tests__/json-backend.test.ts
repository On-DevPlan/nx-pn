...
import { JsonStorageBackend } from '../index.js'
import { runKvBackendContract } from './contract.js'

const roots: string[] = []

async function freshRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'nx-pn-storage-json-'))
  roots.push(root)
  return root
}
...
describe('json backend specifics', () => {
  const descriptor: KvUnitDescriptor = { name: 'shape', version: 1, tables: ['t'], hasGlobal: true }
