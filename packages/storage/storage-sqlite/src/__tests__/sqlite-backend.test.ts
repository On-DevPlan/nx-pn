...
import type { KvUnitDescriptor } from '@flowot/nx-pn-storage'
import { runKvBackendContract } from './contract.js'
import { SqliteStorageBackend, STORAGE_SQLITE_SCHEMA_VERSION, type Config } from '../index.js'

/** Construct the backend over one configuration, applying defaults as the dsh loader did. */
function backendAt(config: Config): SqliteStorageBackend {
  const { path, journalMode } = config
  return new SqliteStorageBackend({ path, ...journalMode === undefined ? {} : { journalMode } })
}
...
async function freshDbPath(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nx-pn-storage-sqlite-'))
  dirs.push(dir)
...
describe('sqlite backend specifics', () => {
  it('opens an in-memory database', async () => {
    const backend = backendAt({ path: ':memory:' })
  ...
  it('propagates filesystem errors other than an existing database file', async () => {
    ...
    const backend = backendAt({ path: join(dir, 'storage.db') })
  ...
  it('propagates an invalid database filename before opening SQLite', async () => {
    const path = await freshDbPath()
    const backend = backendAt({ path: `${path}\0invalid` })
