 ...
 * suite asserts every clause of the `backend.ts` contract so both
 * backends are held to identical semantics.
 *
 * Copied from the dsh storage family contract suite
 * (`deepseek-harness/packages/storage/storage/tests/contract.ts`). This copy
 * lives in the SQLite backend package's `src/__tests__/` because cross-package
 * relative imports are not wired in this workspace; the JSON package carries
 * its own copy under the same path.
 * @module
 */

import { describe, expect, it } from 'vitest'
import type { KvUnitDescriptor, StorageBackend } from '@flowot/nx-pn-storage'
