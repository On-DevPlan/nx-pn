 ...
 * FIRST, then mutates memory, then emits `domain/changed` through the injected
 * emit port — a rejected backend write leaves memory untouched (no divergence
 * between reads and the medium), and events carry values that equal the
 * in-memory state at emission, in write order.
 *
 * Ports replace the cordis context the dsh plugin used: `emit` stands in for
 * `ctx.emit('domain/changed', ...)` and `logger` for `ctx.logger.warn`. The
 * host wires these (broadcasting to WS, forwarding to a ring buffer, …).
 * @module @flowot/nx-pn-storage-domain/src/domain
 */

import type { KvUnit } from '@flowot/nx-pn-storage'
import { DomainError } from './error.js'
import type { DomainSpec, DomainGlobalSpec, TableKeyOf, TableValueOf } from './spec.js'
import type { DomainChanged } from './events.js'

/**
 * Emits one durable domain change. Stands in for the dsh cordis
 * `ctx.emit('domain/changed', change)`; events arrive in the domain's
 * write-chain order.
 */
export type DomainEmitter = (change: DomainChanged) => void

/** Diagnostic logging port standing in for the dsh cordis `ctx.logger`. */
...
export interface Domain<S extends DomainSpec> {
   ...
   * teardown. The consumer owns this call; the facility closes any domain
   * left open when it shuts down.
...
export class DomainImpl {
   ...
   * @param ports - `emit` dispatches one post-durability change notification;
   * `logger` receives warning/error diagnostics.
   */
  constructor(
    spec: DomainSpec,
    ...
    private readonly ports: { emit: DomainEmitter; logger: DomainLogger },
  ) {
    this.name = spec.name
  ...
  private emitChanged(change: DomainChanged): void {
    try {
      ...
      this.ports.logger.warn(`domain '${this.name}': domain/changed listener failed: ${String(error)}`)
    }
  }
