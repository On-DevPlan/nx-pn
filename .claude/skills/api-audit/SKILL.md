...
| Path | Role |
|---|---|
| `apps/cli` | `npx @flowot/nx-pn` entry — start web, `init`/`init-plugin`/`add`/`uninstall`/`build`/`audit`/`plugin` subcommands |
...
| `packages/hmr` | `@flowot/nx-pn-hmr` — standalone watch+rebuild+hot-upload class (private, monorepo-internal; scaffolded workspaces get the logic inlined in their `scripts/dev.mjs` instead) |
| `packages/storage/*` | Pure-library KV storage family (`storage` hub + `json`/`sqlite` backends + `domain` schemas) — zero cordis |
...
Build artifact location: `scripts/build.mjs <id>` writes `<workspace>/dist/<id>.zip`
(workspace-root `dist/`, not per-plugin).
