 ...
 *      (CRC32 + local header + central directory + EOCD, same algorithm as
 *       packages/host/src/plugins/loader.ts:346-399)
 *   6. Write dist/<pluginId>.zip
 */
...
// scripts/build.mjs → go up to project root
...
// Use esbuild from parent monorepo (hardcoded for this test setup)
const MONOREPO_ROOT = 'D:/code/a_js/proj/nx-pn'
const esbuildPath = join(MONOREPO_ROOT, 'node_modules', '.pnpm', 'esbuild@0.24.2', 'node_modules', 'esbuild', 'lib', 'main.js')
...
// Note: react-router-dom is optional - only check if it's used by the plugin
